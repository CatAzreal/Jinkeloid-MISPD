...
import com.jinkeloid.mispd.scenes.StartScene;
...
public class WndGameInProgress extends Window {
	...
	public WndGameInProgress(final int slot){
		...
		MusicImplantSPD.initializeLoggers(slot);
		...
		Button debug = new Button(){
			...
			protected boolean onLongClick() {
				try {
					...
					MusicImplantSPD.scene().addToFront(new WndMessage("_Debug Info:_\n\n" +
		...
		RedButton cont = new RedButton(Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				MusicImplantSPD.switchScene(InterlevelScene.class);
			}
		};
				...
				MusicImplantSPD.scene().add(new WndOptions(
					...
					protected void onSelect( int index ) {
						if (index == 0) {
							...
							MusicImplantSPD.switchNoFade(StartScene.class);
						}
					}
				} );
			}
