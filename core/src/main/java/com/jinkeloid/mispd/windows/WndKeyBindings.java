	...
	public WndKeyBindings() {
		...
		RedButton btnDefaults = new RedButton(Messages.get(this, "default"), 9){
			...
			protected void onClick() {
				changedBindings = MISPDAction.getDefaults();
				for (BindingItem i : listItems){
					int key1 = 0;
					int key2 = 0;
		...
		private boolean onClick( float x, float y ){
			if (inside(x, y)){
				...
				if (x >= this.x + 3*width()/4 && key1 != 0) {
					MusicImplantSPD.scene().addToFront( new WndChangeBinding(gameAction, this, false, key2, key1));

				//assigning first key
				} else if (x >= this.x + width()/2){
					MusicImplantSPD.scene().addToFront( new WndChangeBinding(gameAction, this, true, key1, key2));
