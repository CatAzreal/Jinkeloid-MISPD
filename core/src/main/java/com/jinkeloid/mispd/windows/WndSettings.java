...
import com.jinkeloid.mispd.ui.OptionSlider;
	...
	private static class DisplayTab extends Component {
		...
		protected void createChildren() {
			...
			if ((int)Math.ceil(2* Game.density) < PixelScene.maxDefaultZoom) {
				optScale = new OptionSlider(Messages.get(this, "scale"),
					...
					protected void onChange() {
						if (getSelectedValue() != MISPDSettings.scale()) {
							MISPDSettings.scale(getSelectedValue());
							MusicImplantSPD.seamlessResetScene();
						}
					}
				};
			...
			if (!DeviceCompat.isDesktop() && PixelScene.maxScreenZoom >= 2) {
				chkSaver = new CheckBox(Messages.get(this, "saver")) {
					...
					protected void onClick() {
						...
						if (checked()) {
							checked(!checked());
							MusicImplantSPD.scene().add(new WndOptions(
								...
								protected void onSelect(int index) {
									if (index == 0) {
										checked(!checked());
										MISPDSettings.powerSaver(checked());
									}
								}
							});
						} else {
				...
				chkSaver.checked( MISPDSettings.powerSaver() );
				add( chkSaver );
			}
			...
			optBrightness = new OptionSlider(Messages.get(this, "brightness"),
				...
				@Override
				protected void onChange() {
					MISPDSettings.brightness(getSelectedValue());
				}
			};
			optBrightness.setSelectedValue(MISPDSettings.brightness());
			add(optBrightness);
			...
			optVisGrid = new OptionSlider(Messages.get(this, "visual_grid"),
				...
				@Override
				protected void onChange() {
					MISPDSettings.visualGrid(getSelectedValue());
				}
			};
			optVisGrid.setSelectedValue(MISPDSettings.visualGrid());
			add(optVisGrid);
		...
		protected void createChildren() {
			...
			btnSplit = new RedButton(Messages.get(this, "split")){
				...
				protected void onClick() {
					...
					MISPDSettings.toolbarMode(Toolbar.Mode.SPLIT.name());
					Toolbar.updateLayout();
				}
			};
			if (MISPDSettings.toolbarMode().equals(Toolbar.Mode.SPLIT.name())) btnSplit.textColor(TITLE_COLOR);
			add(btnSplit);
			...
			btnGrouped = new RedButton(Messages.get(this, "group")){
				...
				protected void onClick() {
					...
					textColor(TITLE_COLOR);
					btnCentered.textColor(WHITE);
					MISPDSettings.toolbarMode(Toolbar.Mode.GROUP.name());
					Toolbar.updateLayout();
				}
			};
			if (MISPDSettings.toolbarMode().equals(Toolbar.Mode.GROUP.name())) btnGrouped.textColor(TITLE_COLOR);
			add(btnGrouped);
			...
			btnCentered = new RedButton(Messages.get(this, "center")){
				...
				protected void onClick() {
					...
					btnGrouped.textColor(WHITE);
					textColor(TITLE_COLOR);
					MISPDSettings.toolbarMode(Toolbar.Mode.CENTER.name());
					Toolbar.updateLayout();
				}
			};
			if (MISPDSettings.toolbarMode().equals(Toolbar.Mode.CENTER.name())) btnCentered.textColor(TITLE_COLOR);
			add(btnCentered);
			...
			chkFlipToolbar = new CheckBox(Messages.get(this, "flip_toolbar")){
				...
				protected void onClick() {
					super.onClick();
					MISPDSettings.flipToolbar(checked());
					Toolbar.updateLayout();
				}
			};
			chkFlipToolbar.checked(MISPDSettings.flipToolbar());
			add(chkFlipToolbar);
			...
			chkFlipTags = new CheckBox(Messages.get(this, "flip_indicators")){
				...
				protected void onClick() {
					super.onClick();
					MISPDSettings.flipTags(checked());
					GameScene.layoutTags();
				}
			};
			chkFlipTags.checked(MISPDSettings.flipTags());
			add(chkFlipTags);
			...
			chkFullscreen = new CheckBox( Messages.get(this, "fullscreen") ) {
				...
				protected void onClick() {
					super.onClick();
					MISPDSettings.fullscreen(checked());
				}
			};
			chkFullscreen.checked(MISPDSettings.fullscreen());
			...
			chkFont = new CheckBox(Messages.get(this, "system_font")){
				...
				protected void onClick() {
					...
					MusicImplantSPD.seamlessResetScene(new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							MISPDSettings.systemFont(checked());
						}
			...
			chkFont.checked(MISPDSettings.systemFont());
			add(chkFont);
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						MusicImplantSPD.scene().addToFront(new WndKeyBindings());
					}
				};
	...
	private static class DataTab extends Component{
		...
		protected void createChildren() {
			...
			chkNews = new CheckBox(Messages.get(this, "news")){
				...
				protected void onClick() {
					super.onClick();
					MISPDSettings.news(checked());
					News.clearArticles();
				}
			};
			chkNews.checked(MISPDSettings.news());
			add(chkNews);
			...
			chkUpdates = new CheckBox(Messages.get(this, "updates")){
				...
				protected void onClick() {
					super.onClick();
					MISPDSettings.updates(checked());
					Updates.clearUpdate();
				}
			};
			chkUpdates.checked(MISPDSettings.updates());
			add(chkUpdates);
			...
			if (!DeviceCompat.isDesktop()){
				chkWifi = new CheckBox(Messages.get(this, "wifi")){
					...
					protected void onClick() {
						super.onClick();
						MISPDSettings.WiFi(checked());
					}
				};
				chkWifi.checked(MISPDSettings.WiFi());
				add(chkWifi);
			}
		}
	...
	private static class AudioTab extends Component {
		...
		protected void createChildren() {
			...
			optMusic = new OptionSlider(Messages.get(this, "music_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					MISPDSettings.musicVol(getSelectedValue());
				}
			};
			optMusic.setSelectedValue(MISPDSettings.musicVol());
			add(optMusic);
			...
			chkMusicMute = new CheckBox(Messages.get(this, "music_mute")){
				...
				protected void onClick() {
					super.onClick();
					MISPDSettings.music(!checked());
				}
			};
			chkMusicMute.checked(!MISPDSettings.music());
			add(chkMusicMute);
			...
			optSFX = new OptionSlider(Messages.get(this, "sfx_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					MISPDSettings.SFXVol(getSelectedValue());
					if (Random.Int(100) == 0){
			...
			optSFX.setSelectedValue(MISPDSettings.SFXVol());
			add(optSFX);
			...
			chkMuteSFX = new CheckBox( Messages.get(this, "sfx_mute") ) {
				...
				protected void onClick() {
					super.onClick();
					MISPDSettings.soundFx(!checked());
					Sample.INSTANCE.play( Assets.Sounds.CLICK );
				}
			};
			chkMuteSFX.checked(!MISPDSettings.soundFx());
			add( chkMuteSFX );
		}
				...
				RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName()), 8){
					...
					protected void onClick() {
						...
						MusicImplantSPD.seamlessResetScene(new Game.SceneChangeCallback() {
							@Override
							public void beforeCreate() {
								MISPDSettings.language(langs.get(langIndex));
								GameLog.wipe();
								Game.platform.resetGenerators();
							}
							@Override
