...
import com.jinkeloid.mispd.services.news.NewsArticle;
...
import com.jinkeloid.mispd.windows.WndTitledMessage;
...
public class NewsScene extends PixelScene {
	...
	private static class NewsInfo extends Component {
		...
		protected void createChildren() {
			...
			if (!News.articlesAvailable()){
				if (MISPDSettings.news()) {
					if (MISPDSettings.WiFi() && !Game.platform.connectedToUnmeteredNetwork()) {
	...
	private static class ArticleButton extends StyledButton {
		...
		protected void onClick() {
			...
			if (article.date.getTime() > MISPDSettings.newsLastRead()){
				MISPDSettings.newsLastRead(article.date.getTime());
			}
			MusicImplantSPD.scene().addToFront(new WndArticle(article));
		}
	}
