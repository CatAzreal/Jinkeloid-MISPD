...
public class CellSelector extends ScrollArea {
	...
	private float zoom( float value ) {
		...
		MISPDSettings.zoom((int) (value - PixelScene.defaultZoom));
		camera.zoom( value );
	...
	private GameAction heldAction = MISPDAction.NONE;
	...
	private Signal.Listener<KeyEvent> keyListener = new Signal.Listener<KeyEvent>() {
		...
		public boolean onSignal(KeyEvent event) {
			...
			if (!event.pressed){
				...
				if (heldAction != MISPDAction.NONE && heldAction == action) {
					resetKeyHold();
					return true;
				} else {
					if (action == MISPDAction.ZOOM_IN){
						zoom( camera.zoom+1 );
						return true;

					} else if (action == MISPDAction.ZOOM_OUT){
						zoom( camera.zoom-1 );
						return true;
					}
				}
	...
	private boolean moveFromAction(GameAction action){
		...
		if (action == MISPDAction.N)  cell += -Dungeon.level.width();
		if (action == MISPDAction.NE) cell += +1-Dungeon.level.width();
		if (action == MISPDAction.E)  cell += +1;
		if (action == MISPDAction.SE) cell += +1+Dungeon.level.width();
		if (action == MISPDAction.S)  cell += +Dungeon.level.width();
		if (action == MISPDAction.SW) cell += -1+Dungeon.level.width();
		if (action == MISPDAction.W)  cell += -1;
		if (action == MISPDAction.NW) cell += -1-Dungeon.level.width();
	...
	public void processKeyHold(){
		if (heldAction != MISPDAction.NONE){
			enabled = true;
			heldTurns++;
