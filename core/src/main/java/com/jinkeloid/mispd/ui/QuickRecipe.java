...
import com.jinkeloid.mispd.items.food.Blandfruit;
import com.jinkeloid.mispd.items.food.Food;
import com.jinkeloid.mispd.items.food.MeatPie;
import com.jinkeloid.mispd.items.food.MysteryMeat;
import com.jinkeloid.mispd.items.food.Pasty;
import com.jinkeloid.mispd.items.food.StewedMeat;
import com.jinkeloid.mispd.items.potions.AlchemicalCatalyst;
import com.jinkeloid.mispd.items.potions.Potion;
import com.jinkeloid.mispd.items.potions.brews.BlizzardBrew;
import com.jinkeloid.mispd.items.potions.brews.CausticBrew;
import com.jinkeloid.mispd.items.potions.brews.InfernalBrew;
import com.jinkeloid.mispd.items.potions.brews.ShockingBrew;
import com.jinkeloid.mispd.items.potions.elixirs.ElixirOfAquaticRejuvenation;
import com.jinkeloid.mispd.items.potions.elixirs.ElixirOfArcaneArmor;
import com.jinkeloid.mispd.items.potions.elixirs.ElixirOfDragonsBlood;
import com.jinkeloid.mispd.items.potions.elixirs.ElixirOfHoneyedHealing;
import com.jinkeloid.mispd.items.potions.elixirs.ElixirOfIcyTouch;
import com.jinkeloid.mispd.items.potions.elixirs.ElixirOfMight;
import com.jinkeloid.mispd.items.potions.elixirs.ElixirOfToxicEssence;
...
import com.jinkeloid.mispd.items.spells.Alchemize;
import com.jinkeloid.mispd.items.spells.AquaBlast;
import com.jinkeloid.mispd.items.spells.ArcaneCatalyst;
import com.jinkeloid.mispd.items.spells.BeaconOfReturning;
import com.jinkeloid.mispd.items.spells.CurseInfusion;
import com.jinkeloid.mispd.items.spells.FeatherFall;
import com.jinkeloid.mispd.items.spells.MagicalInfusion;
import com.jinkeloid.mispd.items.spells.MagicalPorter;
import com.jinkeloid.mispd.items.spells.PhaseShift;
import com.jinkeloid.mispd.items.spells.ReclaimTrap;
import com.jinkeloid.mispd.items.spells.Recycle;
import com.jinkeloid.mispd.items.spells.WildEnergy;
...
import com.jinkeloid.mispd.windows.WndInfoItem;
...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					MusicImplantSPD.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		if (hasInputs) {
			...
			if (!(MusicImplantSPD.scene() instanceof AlchemyScene)) {
				arrow.enable(false);
			}
		} else {
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				MusicImplantSPD.scene().addToFront(new WndInfoItem(output));
			}
		};
		if (!hasInputs){
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene) MusicImplantSPD.scene()).populate(ingredients, Dungeon.hero.belongings);
		}
