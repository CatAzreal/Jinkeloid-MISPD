...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		for(int i = 0; i <= 3; i++) {
			if (i == 0 && !MISPDSettings.flipToolbar() ||
				i == 3 && MISPDSettings.flipToolbar()){
				btnQuick[i].border(0, 2);
				btnQuick[i].frame(106, 0, 19, 24);
			} else if (i == 0 && MISPDSettings.flipToolbar() ||
					i == 3 && !MISPDSettings.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(MISPDSettings.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
