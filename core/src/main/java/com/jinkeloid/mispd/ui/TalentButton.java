...
import com.jinkeloid.mispd.windows.WndInfoTalent;
...
public class TalentButton extends Button {
	...
	public TalentButton(int tier, Perk perk, int points, boolean upgradeEnabled){
		super();
		...
		bg.frame(20*(perk.pointCosts()-1), 0, WIDTH, HEIGHT);
		icon.frame( film.get( perk.icon() ) );
	}
	...
	protected void layout() {
		...
		fill.x = x;
		fill.y = y + WIDTH - 1;
		fill.size( pointsInTalent/(float) perk.pointCosts() * WIDTH, 5);
...
//				&& Dungeon.hero.talentPointsAvailable(tier) > 0
				&& Dungeon.hero.pointsInTalent(perk) < perk.pointCosts()){
			MusicImplantSPD.scene().addToFront(new WndInfoTalent(perk, pointsInTalent, new Callback() {
				@Override
				public void call() {
					upgradeTalent();
				}
			...
			MusicImplantSPD.scene().addToFront(new WndInfoTalent(perk, pointsInTalent, null));
		}
	}
...
//			Dungeon.hero.upgradeTalent(perk);
