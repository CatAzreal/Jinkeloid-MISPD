...
public class TalentsPane extends ScrollPane {
	...
	public TalentsPane( boolean canUpgrade ) {
		this( canUpgrade, Dungeon.hero.placeholder);
	}

	public TalentsPane( boolean canUpgrade, ArrayList<LinkedHashMap<Perk, Integer>> talents ) {
		super(new Component());

		int tiersAvailable = 1;

		if (!canUpgrade){
			tiersAvailable = Perk.MAX_TALENT_TIERS;
		} else {
			while (tiersAvailable < Perk.MAX_TALENT_TIERS
					&& Dungeon.hero.lvl+1 >= Perk.tierLevelThresholds[tiersAvailable+1]){
				tiersAvailable++;
			}
	...
	public static class TalentTierPane extends Component {
		...
		public TalentTierPane(LinkedHashMap<Perk, Integer> talents, int tier, boolean canUpgrade){
			super();
			...
			for (Perk perk : talents.keySet()){
				TalentButton btn = new TalentButton(tier, perk, talents.get(perk), canUpgrade){
					@Override
		...
		private void setupStars(){
			...
			int totStars = Perk.tierLevelThresholds[tier+1] - Perk.tierLevelThresholds[tier];
