...
public class Momentum extends Buff implements ActionIndicator.Action {
	...
	public boolean act() {
		...
		if (freerunCooldown == 0 && target.invisible > 0 && Dungeon.hero.pointsInTalent(Perk.SPEEDY_STEALTH) >= 1){
			momentumStacks = Math.min(momentumStacks + 2, 10);
			movedLastTurn = true;
		}

		if (freerunTurns > 0){
			if (target.invisible == 0 || Dungeon.hero.pointsInTalent(Perk.SPEEDY_STEALTH) < 2) {
				freerunTurns--;
			}
	...
	public float speedMultiplier(){
		if (freerunning()){
			return 2;
		} else if (target.invisible > 0 && Dungeon.hero.pointsInTalent(Perk.SPEEDY_STEALTH) == 3){
			return 2;
		} else {
			return 1;
		}
	...
	public int evasionBonus( int heroLvl, int excessArmorStr ){
		if (freerunTurns > 0) {
			return heroLvl/2 + excessArmorStr*Dungeon.hero.pointsInTalent(Perk.EVASIVE_ARMOR);
		} else {
			return 0;
		}
	}
