...
import com.jinkeloid.mispd.windows.WndCombo;
...
public class Combo extends Buff implements ActionIndicator.Action {
	...
	public void hit( Char enemy ) {
		...
		if (!enemy.isAlive() || (enemy.buff(Corruption.class) != null && enemy.HP == enemy.HT)){
			comboTime = Math.max(comboTime, 10*((Hero)target).pointsInTalent(Perk.CLEAVE));
		}
	...
	public Image getIcon() {
		Image icon;
		if (((Hero)target).belongings.mainhand != null){
			icon = new ItemSprite(((Hero)target).belongings.mainhand.image, null);
		} else {
			...
			switch (moveBeingUsed) {
				case CLOBBER:
					...
					if (enemy.isAlive()) {
						...
						int dist = 2;
						if (count >= 7 && hero.pointsInTalent(Perk.ENHANCED_COMBO) >= 1){
							dist ++;
	...
	private CellSelector.Listener listener = new CellSelector.Listener() {
		...
		public void onSelect(Integer cell) {
			...
			if (enemy == null
				...
				if (((Hero) target).pointsInTalent(Perk.ENHANCED_COMBO) < 3
