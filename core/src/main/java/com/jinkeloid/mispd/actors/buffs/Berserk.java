...
import com.jinkeloid.mispd.items.BrokenSeal.WarriorShield;
...
public class Berserk extends Buff {
	...
	public boolean act() {
		if (berserking()){
			...
			if (target.HP <= 0) {
				...
				levelRecovery = LEVEL_RECOVER_START - Dungeon.hero.pointsInTalent(Perk.BERSERKING_STAMINA)/3f;
	...
	public boolean berserking(){
		if (target.HP == 0 && state == State.NORMAL && power >= 1f){
			...
			if (shield != null){
				...
				shieldAmount = Math.round(shieldAmount * (1f + Dungeon.hero.pointsInTalent(Perk.BERSERKING_STAMINA)/6f));
	...
	public void damage(int damage){
		...
		float maxPower = 1f + 0.15f*((Hero)target).pointsInTalent(Perk.ENDLESS_RAGE);
