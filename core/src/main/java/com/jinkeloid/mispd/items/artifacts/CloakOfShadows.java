...
public class CloakOfShadows extends Artifact {
		...
		if (action.equals( AC_STEALTH )) {
			...
			if (activeBuff == null){
				if (!isEquipped(hero) && !hero.hasTalent(Perk.LIGHT_CLOAK)) GLog.i( Messages.get(Artifact.class, "need_to_equip") );
					...
					Perk.onArtifactUsed(Dungeon.hero);
	...
	public void charge(Hero target, float amount) {
		if (charge < chargeCap) {
			if (!isEquipped(target)) amount *= target.pointsInTalent(Perk.LIGHT_CLOAK)/10f;
			...
			if (charge < chargeCap) {
				...
				if (activeBuff == null && (lock == null || lock.regenOn())) {
					...
					if (!isEquipped(Dungeon.hero)){
						chargeToGain *= 0.1f*Dungeon.hero.pointsInTalent(Perk.LIGHT_CLOAK);
					}
