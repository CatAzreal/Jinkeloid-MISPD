...
package com.jinkeloid.mispd.items.weapon.missiles;
...
abstract public class MissileWeapon extends Weapon {
	...
	public int throwPos(Hero user, int dst) {
		...
		if (!projecting && Random.Int(3) < user.pointsInTalent(Perk.SHARED_ENCHANTMENT)){
	...
	public float accuracyFactor(Char owner) {
		...
		if (owner instanceof Hero && owner.buff(Momentum.class) != null && owner.buff(Momentum.class).freerunning()){
			accFactor *= 1f + 0.2f*((Hero) owner).pointsInTalent(Perk.PROJECTILE_MOMENTUM);
		}
		return accFactor;
	}
	...
	public int proc(Char attacker, Char defender, int damage) {
		if (attacker == Dungeon.hero && Random.Int(3) < Dungeon.hero.pointsInTalent(Perk.SHARED_ENCHANTMENT)){
			if (this instanceof Dart && ((Dart) this).crossbowHasEnchant(Dungeon.hero)){
				//do nothing
			} else {
				SpiritBow bow = Dungeon.hero.belongings.getItem(SpiritBow.class);
	...
	protected float durabilityPerUse(){
		...
		//+50%/75% durability
		if (Dungeon.hero.hasTalent(Perk.DURABLE_PROJECTILES)){
			usages *= 1.25f + (0.25f*Dungeon.hero.pointsInTalent(Perk.DURABLE_PROJECTILES));
		}
		if (holster) {
	...
	public int damageRoll(Char owner) {
		...
		if (owner instanceof Hero) {
			...
			if (owner.buff(Momentum.class) != null && owner.buff(Momentum.class).freerunning()) {
				damage = Math.round(damage * (1f + 0.15f * ((Hero) owner).pointsInTalent(Perk.PROJECTILE_MOMENTUM)));
			}
		}
