...
package com.jinkeloid.mispd.items.weapon.melee;
...
public class MagesStaff extends MeleeWeapon {
	...
	public int proc(Char attacker, Char defender, int damage) {
		if (attacker.buff(Perk.EmpoweredStrikeTracker.class) != null){
			attacker.buff(Perk.EmpoweredStrikeTracker.class).detach();
			damage = Math.round( damage * (1f + Dungeon.hero.pointsInTalent(Perk.EMPOWERED_STRIKE)/5f));
		}

		if (wand.curCharges >= wand.maxCharges && attacker instanceof Hero && Random.Int(5) < ((Hero) attacker).pointsInTalent(Perk.EXCESS_CHARGE)){
		...
		public void onSelect( final Item item ) {
			if (item != null) {
				...
				if (wand == null){
					...
					if (preservesLeft > 0){
						int preserveChance = Dungeon.hero.pointsInTalent(Perk.WAND_PRESERVATION) == 1 ? 67 : 100;
