...
public class Candle extends Lightsource{
    ...
    public void updateSprite() {
        super.updateSprite();
        image = isActivated() ? ItemSpriteSheet.CANDLELIT : ItemSpriteSheet.CANDLE;
    }
...
//                curUser = hero;
//                curItem = this;
//                GameScen.e.selectCell( burner );
//            } else {
//                GLog.w( TXT_CANT_BURN );
//            }
        } else if (action.equals( AC_UNEQUIP ) && this.isActivated()) {
    ...
    public void activate( Hero hero, boolean voluntary ) {
        ...
        Buff.affect( hero, Light.CandleLight.class );
    ...
    public void deactivate( Hero hero, boolean voluntary ) {
        ...
        active = false;
        updateSprite();
        hero.buff( Light.CandleLight.class ).ManualDetach();
        Sample.INSTANCE.play( Assets.Sounds.CLICK );
        updateQuickslot();
    ...
    public String desc() {
        return super.desc() +
        (charge <= 0 ? Messages.get(this, "state_depleted") :
