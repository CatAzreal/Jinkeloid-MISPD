...
import com.jinkeloid.mispd.actors.buffs.ScrollEmpower;
...
public abstract class Wand extends Item {
	...
	public int buffedLvl() {
		...
		if (charger != null && charger.target != null) {
			if (charger.target.buff(ScrollEmpower.class) != null){
				lvl += Dungeon.hero.pointsInTalent(Perk.EMPOWERING_SCROLLS);
			}
	...
	protected void wandUsed() {
		if (!isIdentified()) {
			...
			usesLeftToID -= uses;
			if (usesLeftToID <= 0 || Dungeon.hero.pointsInTalent(Perk.SCHOLARS_INTUITION) == 2) {
				identify();
