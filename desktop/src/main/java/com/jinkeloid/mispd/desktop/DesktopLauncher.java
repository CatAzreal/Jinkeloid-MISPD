...
package com.jinkeloid.mispd.desktop;
...
public class DesktopLauncher {
	...
	public static void main (String[] args) {
		...
		FileHandle newPrefs = new Lwjgl3FileHandle(basePath + MISPDSettings.DEFAULT_PREFS_FILE, Files.FileType.External);
		if (oldPrefs.exists() && !newPrefs.exists()){
			oldPrefs.copyTo(newPrefs);
		}

		config.setPreferencesConfig( basePath, Files.FileType.External );
		MISPDSettings.set( new Lwjgl3Preferences( MISPDSettings.DEFAULT_PREFS_FILE, basePath) );
		FileUtils.setDefaultFileProperties( Files.FileType.External, basePath );
		
		config.setWindowSizeLimits( 480, 320, -1, -1 );
		Point p = MISPDSettings.windowResolution();
		...
		new Lwjgl3Application(new MusicImplantSPD(new DesktopPlatformSupport()), config);
	}
}
